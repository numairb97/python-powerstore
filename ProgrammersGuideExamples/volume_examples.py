...
# Map volume to Host
CONN.provisioning.map_volume_to_host(volume_id=VOL[0]["id"], host_id=HOST["id"])

# Get Host Volume mapping information
HOST_VOLUME_MAPPING = CONN.provisioning.get_host_volume_mapping(volume_id=VOL[0]["id"])
print(HOST_VOLUME_MAPPING)

# Unmap volume from Host
CONN.provisioning.unmap_volume_from_host(volume_id=VOL[0]["id"], host_id=HOST["id"])
...
# Get volume details
VOL_DETAILS = CONN.provisioning.get_volume_details(volume_id=VOL[0]["id"])
print(VOL_DETAILS)
...
# Clone Volume
CLONE_VOLUME = CONN.provisioning.clone_volume(
    volume_id=VOL[0]["id"],
